...
def main(args):
    if not len(args) >= 4: sys.exit("USAGE: python addAverageCountsToGene.py ALYkd5.txt hg19_TSS.txt ~/Desktop/Alexander2023_filesTooBigForGithub/SONdata/counts* > ALYkd5_withSONcounts.txt")


    f = open(args[1])
    geneFileDict = {}
    line = f.readline()[:-1]
    header = "Gene\t" + "\t".join(line.split("\t")[1:]) + "\tSONdmso1\tSONdmso2\tSONpt2399-1\tSONpt2399-2"
    line = f.readline()[:-1]
    while line != "":
        geneFileDict[line.split("\t")[0]] = "\t".join(line.split("\t")[1:])
        line = f.readline()[:-1]
    f.close()
